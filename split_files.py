...
def get_file_pairs_from_txt_files(im_txt_file, lbl_txt_filem just_labelled):
    
    ims = open(im_txt_file,"r").readlines()
    lbls = open(lbl_txt_file,"r").readlines()
    if just_labelled:
        ims = [im for im in ims if "climo" in im]
        lbls = [lbl for lbl in lbls if "climo" in lbl]
...
def save_n_file_lists(im_txt_file, lbl_txt_file, n, dest_dir, num_crops, master_file_path, typ="tr", constant_num_for_each=None, just_labelled=False):
    dest_dir = make_filelist_dir(dest_dir, n, master_file_path)
    if not exists(dest_dir):
        os.makedirs(dest_dir)
    print n
    def make_n_file_pairs_lists(im_txt_file, lbl_txt_file, n):
        file_pairs = get_file_pairs_from_txt_files(im_txt_file=im_txt_file, lbl_txt_file=lbl_txt_file, just_labelled)
...
if __name__ == "__main__":
    cl_args = { "num_nodes": 32,
            ...
            "files_dest_dir": "/project/projectdirs/dasrepo/gordon_bell/deep_learning/networks/climate/2d_semi_sup/test_with_crops/small_medium/",
    ...
    if cla["file_split"]:
        ...
        save_n_file_lists(im_txt_file=im_txt_file, 
                         ...
                         num_crops=cla["num_crops"],master_file_path=cla["master_file_path"], typ="tr", 
                          constant_num_for_each=cla["constant_num_for_each"], cla["just_labelled"])
